...
export default function ExperienceCard({ url, title, fullDescription, cardDescription, cardImage, media, delay, gradient, myRole, timeline }: { url?: string, title: string, fullDescription: string[], cardDescription: string, cardImage: string, media: string[], delay: number, gradient: string, myRole: string, timeline: string }) {
    ...
    return (
        <>
            ...
            >
                <div
                    className={`p-4 flex md:flex-row flex-col gap-6 ${gradient} from-primary to-secondary rounded-lg border-1 border-accent shadow-2xl shadow-background items-center`}>
                    <img alt="" draggable={false}
                         className="rounded-lg md:h-[14rem] sm:h-[12rem] h-[10rem] grayscale group-hover:grayscale-0 duration-300"
                         src={cardImage}/>
                    <div className="flex flex-col">
                        ...
                        <div className="flex row gap-4 mt-2">
                            ...
                            <Button label="View More" onClick={() => setModalOpen(true)}
                    ...
                    <Modal open={modalOpen} setOpen={setModalOpen}>
                        ...
                        <div className="flex lg:flex-row flex-col justify-between mt-6 px-3">
                                ...
                                <div className="flex flex-row gap-2 items-center">
                                    ...
                                    {url &&
                                        <Link href={url} target="_blank"
                                            ...
                                            <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 fill-white"
                                                 viewBox="0 0 16 16">
                                                <path
                            ...
                            <div className="flex flex-col">
                                ...
                                <div
                                    className="max-h-[16.5rem] overflow-y-scroll bg-neutral-800 border-1 border-accent rounded-lg p-2">
                                    {fullDescription.map((desc, i) => (
                                        <p key={i}
                                           className="sm:text-lg text-base first:mt-0 mt-2 max-w-[28rem]">{desc}</p>
                                    ))}
                                </div>
                            </div>
                        </div>
